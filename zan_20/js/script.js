...
person = { ...preson }; //копирование обькта    спред оператор

const { name } = person; // берет у обькта свойтво
const arr = [1, 2, 3, 4];
const [a, , b] = arr; //a 0 элемент  и b  2 элемент

function r({ name }) {
  console.log("hello" + name);
}
r(person); // person обькт  со свойством name диструктуризация


set уникальный  значение  map  обьеты в виде свойства  weackset и  weackmap      
